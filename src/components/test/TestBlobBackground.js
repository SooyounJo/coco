...
function MiniScene({ opacity, boost, paletteMix }) {
  // Use a "stable-saturation" variant for UI badges so colors don't pulse/desaturate.
  const material = useMemo(() => createMiniShaderMaterial(), []);
...
// Mini-blob shader: based on default, but removes the time-based saturation/brightness/contrast pulsing
// so the badge doesn't "lose saturation" periodically.
function createMiniShaderMaterial() {
  return new THREE.ShaderMaterial({
    uniforms: {
      time: { value: 0 },
      lightDir: { value: new THREE.Vector3(0.2, 0.9, 0.3).normalize() },
    ...
    fragmentShader: `
      ...
      void main(){
        ...
        lit+=glow;
        vec3 gray=vec3(dot(lit,vec3(0.299,0.587,0.114)));
        // Constant saturation/contrast/brightness (no pulsing)
        lit = mix(gray, lit, 1.85);
        lit=pow(lit,vec3(0.92)); lit*=mix(1.0,1.05,boost); lit=mix(lit,vec3(1.0),0.02); lit=clamp(lit,0.0,1.0);
        float edgeBase = smoothstep(0.56, 0.32, r);
        float edgeGlow = softBlur(r - 0.4, 0.15);
        float edgeFeather = edgeBase * (1.0 + edgeGlow * 0.3);
